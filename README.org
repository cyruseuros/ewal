...
If you prefer =spacemacs-theme= with borders, simply pass the 4_th, =BORDERS=
argument to =ewal-get-spacemacs-theme-colors= like so:
#+BEGIN_SRC emacs-lisp :tangle yes
  (ewal-get-spacemacs-theme-colors :borders '(t))
#+END_SRC
The argument still needs to be a list due to some implementation details I will
resolve promptly (suggestions for elegant solutions are welcome).
